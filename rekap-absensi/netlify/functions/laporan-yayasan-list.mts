...
export default amankan(async (req: Request) => {
  ...
  const rows = await db().sql`
    SELECT ly.id, ly.judul, ly.cabang, ly.jenjang, ly.tgl_mulai, ly.tgl_selesai, ly.sumber_file, ly.diperbarui_pada,
           COUNT(lyb.id)::int AS jumlah_baris
    FROM laporan_yayasan ly
    LEFT JOIN laporan_yayasan_baris lyb ON lyb.laporan_id = ly.id
    GROUP BY ly.id
    ORDER BY ly.tgl_mulai DESC, ly.cabang, ly.jenjang
  `;
